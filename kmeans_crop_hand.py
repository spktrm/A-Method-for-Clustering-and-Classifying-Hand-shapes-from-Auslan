...
file_count = sum(len(files) for _, _, files in os.walk(file + "/kmeans_right_hand_clusters"))
...
with alive_bar(file_count) as bar:
    for root, dirs, files in os.walk(file + "/kmeans_right_hand_clusters"):
        for f in files:
            if f.endswith(".jpg"):
...
file_count = sum(len(files) for _, _, files in os.walk(file + "/kmeans_left_hand_clusters"))
...
frame_no = 0
with alive_bar(file_count) as bar:
    for root, dirs, files in os.walk(file + "/kmeans_left_hand_clusters"):
        for f in files:
            if f.endswith(".jpg"):
                image = cv2.imread(os.path.join(root, f))
                keypoints = df[int(f.split('.')[0])]
